...
// field, using a GeoJSON multilinestring representation. Just give it a set
...
export const Line = Backbone.Model.extend({
  urlRoot: '/api/lines',

  defaults: function() {
    var color = utils.getNextColor();
    var name = _.random(10, 99) + ' ' + utils.getRandomName();
  ...
  parse: function(response) {
    ...
    return utils.removeUndefined(attrs);
  },
  ...
  _updateMiddleWaypoint: function(latlng, index, ignoreRoads) {
    ...
    utils.getRoute({
      ...
      var closest = utils.indexOfClosest(route, latlng);
  ...
  getCalculations: function() {
    ...
    var distance = utils.calculateDistance(latlngs) * 2;
    ...
    var calculate = function(sw) {
      ...
      var minutesPerDay = utils.diffTime(sw.get('from'), sw.get('to'));
