...
export const MapController = Controller.extend({
  ...
  setupViews: function(lineId) {
    ...
    this.linesView = new CollectionView({
  ...
  showNearby: function(latlng) {
    ...
    utils.getNearbyGTFS(latlng, function(nearby) {
  ...
  addNearbyLine: function(line) {
    utils.getNearbyCoordinates(line.get('mapId'), line.id, function(coordinates) {
      var lines = this.map.get('lines');
