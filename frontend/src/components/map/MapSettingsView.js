// Map Settings View Component
import { BaseView } from '../../framework/BaseView.js';
import { CollectionView } from '../../framework/CollectionView.js';
import { mapSettingsViewTemplate } from '../../templates/map-settings-view.js';
import { app } from '../../app.js';
import * as utils from '../../utils.js';

export const MapSettingsView = BaseView.extend({
  templateString: mapSettingsViewTemplate,
    ...
    '.speed': {
      ...
      onGet: function(values) {
        if (this.model.get('preferMetricUnits')) {
          return utils.milesToKilometers(values[0]).toFixed(1) + ' km/h';
        } else {
      ...
      onSet: function(val) {
        var distance;
        if (this.model.get('preferMetricUnits')) {
          distance = utils.kilometersToMiles(parseFloat(val));
        } else {
  ...
  views: function() {
    ...
    var weekdays = new CollectionView({
    ...
    var weekends = new CollectionView({
      collection: serviceWindows,
      view: app.ServiceWindowView,
      viewOptions: { showToggle: true },
      filter: function(item) { return item.get('isSaturday') || item.get('isSunday'); },
...
if (typeof window !== 'undefined') {
  ...
  window.app.MapSettingsView = MapSettingsView;
}
